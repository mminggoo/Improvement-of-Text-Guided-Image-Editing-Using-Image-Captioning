...
for data in TEDbench_datas:
    ...
    inputs = processor(Image.open(SOURCE_IMAGE_PATH), data['object'] + ' in', return_tensors="pt").to(device)
    out = captioner.generate(**inputs)

    target_prompt = data['target_text'] + ', background is ' + processor.decode(out[0], skip_special_tokens=True)[len(data['object']):]
